...
from ModelC_TSV.Funcs.ranking import db_rank_ts as tsvector_ranking
    ...
    for i in range(0, iterations):
        ...
        for row in res:
            if type(row) is tuple:
                row = row[0]
            if 'Execution Time' in row:
                executes.append(float(re.findall("\d+\.\d+", row)[0]))
            elif 'Planning Time' in row:
                pannings.append(float(re.findall("\d+\.\d+", row)[0]))
    return sum(executes)/len(executes), sum(pannings)/len(pannings)
...
def analyse_query_time_modelc(query, iterations=50):
    ...
    print("******Ranking SPEED******")
    execution_time, planning_time = perform_iterations(True, iterations, partial(tsvector_ranking,
                                        query.replace("OR", "|").replace("AND", "&"), True))
    print("ExecutionTimeC_tsrank: " + str(execution_time) + " PlanningTimeC_tsrank: " + str(planning_time))


def test_speed():
    analyse_query_time_modela('trump', 20)
    #explain_query_modela('trump')
    #analyse_query_time_modelb('trump', 20)
    #explain_query_modelb('trump')
